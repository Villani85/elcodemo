...
This script generates the flow metadata XML programmatically to avoid manual repetition.
"""
...
def generate_assignment_element(profile_name, spec_num, category, parameter, value):
    """Generate a single Assignment element for a spec."""
    element_name = f"Build_{profile_name}_Spec_{spec_num:02d}"
    y_location = 566 + (spec_num - 1) * 108

    # Determine next connector
    if spec_num < 19:
        next_element = f"Build_{profile_name}_Spec_{spec_num + 1:02d}"
    else:
        next_element = f"Create_All_{profile_name}_Specs"

    return f'''
    <!-- {profile_name} SPEC {spec_num:02d}: {category} / {parameter} = "{value}" -->
    <assignments>
        <name>{element_name}</name>
        <label>Build {profile_name} Spec {spec_num:02d}</label>
...
def generate_record_create_element(profile_name):
    """Generate the RecordCreate element for a profile."""
    y_location = 566 + 19 * 108  # After last assignment

    return f'''
    <!-- CREATE ALL {profile_name.upper()} SPECS -->
    <recordCreates>
...
def generate_complete_flow():
    """Generate the complete Flow XML."""
...
'''
    ...
    # Generate assignments for all profiles
    for profile_key, specs in PCB_PROFILES.items():
        profile_name = profile_key.replace("_", "").capitalize()
        if profile_key == "HIGH_TG":
            profile_name = "HighTg"

        flow_xml += f'\n    <!-- ===== {profile_key} PCB SPECS (19 assignments) ===== -->'

        for idx, (category, parameter, value) in enumerate(specs, start=1):
            flow_xml += generate_assignment_element(profile_name, idx, category, parameter, value)

        flow_xml += generate_record_create_element(profile_name)
